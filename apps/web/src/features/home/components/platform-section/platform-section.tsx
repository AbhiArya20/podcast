  ...
  const onTabClick = (index) => {
    if (cardItemsRef.current.length > index)
      cardItemsRef.current[index].scrollIntoView({
        behavior: "smooth",
        block: index < tabs.length - 1 ? "center" : "nearest",
      });
  };
        ...
        <div className={styles.plateformSection}>
          <div className={`${styles.platerfromSectionLeft}`}>
            <div className={styles.stickyCard}>
              <ImageSection
                alt="Zoho Webinar"
                className={styles.plateformMainImage}
                src="/assets/person.jpeg"
              />
              <div className={styles.plateformImagesWrapper}>
                <ImageSection
                  alt="Meeting 1"
                  className={`${styles.plateformSingleImage} ${styles.meeting1} ${activeTab === 0 && styles.activeMeeting1}`}
                  src="assets/meeting1.png"
                />
                <ImageSection
                  alt="Meeting 2"
                  className={`${styles.plateformSingleImage} ${styles.meeting2} ${activeTab === 0 && styles.activeMeeting2}`}
                  src="/assets/meeting2.png"
                />
                <ImageSection
                  alt="Rooms 1"
                  className={`${styles.plateformSingleImage} ${styles.room1} ${activeTab === 3 && styles.activeRoom1}`}
                  src="/assets/rooms1.png"
                />
                <ImageSection
                  alt="Rooms 2"
                  className={`${styles.plateformSingleImage} ${styles.room2} ${activeTab === 3 && styles.activeRoom2}`}
                  src="/assets/rooms2.png"
                />
                <ImageSection
                  alt="Webinar 1"
                  className={`${styles.plateformSingleImage} ${styles.webinar1} ${activeTab === 2 && styles.activeWebinar1}`}
                  src="/assets/webinar1.png"
                />
                <ImageSection
                  alt="Webinar 2"
                  className={`${styles.plateformSingleImage} ${styles.webinar2} ${activeTab === 2 && styles.activeWebinar2}`}
                  src="/assets/webinar2.png"
                />
                <ImageSection
                  alt="Calls"
                  className={`${styles.plateformSingleImage} ${styles.calls} ${activeTab === 1 && styles.activeCalls}`}
                  src="/assets/calls.png"
                />
              </div>
            </div>
          </div>
              ...
              <div className={styles.plateFromTextSubSection}>
                <ul>
                  {content.map((item, tabIdx) => (
                    <li ref={(el) => (cardItemsRef.current[tabIdx] = el)}>
                      <CardList
                        activeTab={activeTab}
...
const CardList = ({ activeTab, setActiveTab, item, tabIdx }) => {
  ...
  return (
    <div ref={ref} style={{ display: "flex", flexDirection: "column" }}>
      <h2
        style={{ marginTop: tabIdx !== 0 ? "2rem" : 0 }}
        className={styles.nonStickyOptions}
        id={"#" + item.title}
      >
        {item.title}
      </h2>
      <ul style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
        {item.icons.map((icon, index) => (
          <CardItem
            key={index}
            isActive={activeTab === tabIdx}
            icon={icon}
            title={item.texts[index]}
...
const CardItem = ({ isActive, icon, title, description }) => (
  <li>
    <div
      className={`${styles.subSectionCardHeadingWrapper} ${!isActive && styles.subSectionCardHeadingWrapperDissabled}`}
    >
      <span>{icon}</span>
      <h4>{title}</h4>
    </div>
    <p
      className={`${styles.subSectionCardHeadingText} ${!isActive && styles.subSectionCardHeadingTextDissabled}`}
    >
      {description}
    </p>
  </li>
